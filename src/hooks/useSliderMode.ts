...
export const useSliderMode = ({
  ...
  useEffect(() => {
    ...
    function assignSlot(slot: SlideData, projectIdx: number) {
      ...
      const isAnimating =
        (collapsingRef.current &&
          slot.projectIndex === collapseProjectIndexRef.current) ||
        (revealingRef.current &&
          slot.projectIndex === revealedIndexRef.current);
      if (isAnimating) {
    ...
    const tick = () => {
      ...
      if (!isDraggingRef.current && !jumpTweenRef.current) {
        inertiaVelocityRef.current = Math.max(
          -MAX_INERTIA,
          Math.min(MAX_INERTIA, inertiaVelocityRef.current),
        );
        scrollTargetRef.current += inertiaVelocityRef.current;
        inertiaVelocityRef.current *= 0.95;
        if (Math.abs(inertiaVelocityRef.current) < 0.0001)
          inertiaVelocityRef.current = 0;
      }
      ...
      if (jumpTweenRef.current) {
        ...
        scrollRef.current +=
          (scrollTargetRef.current - scrollRef.current) * SCROLL_LERP;
      }
      const rawVel = scrollRef.current - prevScroll;
      scrollVelocityRef.current = Math.max(
        -MAX_SCROLL_VELOCITY,
        Math.min(MAX_SCROLL_VELOCITY, rawVel),
      );
      ...
      for (let i = 0; i < actualCount; i++) {
        ...
        slides[i].program.uniforms.uTextureReady.value +=
          (targetReady - cur) * 0.08;

        // Per-slide distortion (clamped)
        const distFromCenter = Math.abs(y) / (curCtx.viewport.height / 2);
        slides[i].program.uniforms.u_distortionAmount.value = Math.min(
          MAX_DISTORTION,
          Math.abs(scrollVelocityRef.current) *
            (1 - Math.min(distFromCenter, 1)) *
            SLIDE_DISTORTION_MULT,
        );
      ...
      if (revealingRef.current || collapsingRef.current) {
        ...
        if (revealingRef.current) {
          const revSlide = slides.find(
            (s) => s.projectIndex === revealedIndexRef.current,
          );
          if (revSlide) {
            const raw = Math.min(
              (now - revealStartTimeRef.current) /
                1000 /
        ...
        if (collapsingRef.current) {
          const colSlide = slides.find(
            (s) => s.projectIndex === collapseProjectIndexRef.current,
          );
          if (colSlide) {
            const raw = Math.min(
              (now - collapseStartTimeRef.current) / 1000 / COLLAPSE_DURATION,
              1,
            );
            const t = easeOutCubic(raw);
            const cw =
              collapseStartScaleRef.current.w +
              (nomW - collapseStartScaleRef.current.w) * t;
            const ch =
              collapseStartScaleRef.current.h +
              (nomH - collapseStartScaleRef.current.h) * t;
      ...
      const hasExpanded =
        revealedRef.current || revealingRef.current || collapsingRef.current;
      const expandedIdx = collapsingRef.current
        ? collapseProjectIndexRef.current
        : revealedIndexRef.current;

      if (hasExpanded) {
        const expandedSlide = slides.find(
          (s) => s.projectIndex === expandedIdx,
        );
        if (expandedSlide) {
      ...
      if (hasExpanded && revealBoundsRef) {
        ...
        if (bSlide) {
          ...
          const sx =
            ((mx - mw / 2 + curCtx.viewport.width / 2) /
              curCtx.viewport.width) *
            cw;
          const sy =
            ((curCtx.viewport.height / 2 - my - mh / 2) /
              curCtx.viewport.height) *
            ch;
      ...
      if (postfxMeshRef.current) {
        ...
        const target = Math.min(
          MAX_POSTFX_DISTORTION,
          Math.abs(scrollVelocityRef.current) * VELOCITY_MULTIPLIER,
        );
        prog.uniforms.u_distortionAmount.value +=
          (target - prog.uniforms.u_distortionAmount.value) * VELOCITY_LERP;
      }

      curCtx.renderer.render({
        scene: slidesScene,
        camera: curCtx.camera,
        target: rt,
      });
    };
    ...
    function computeRevealTarget(projectIndex: number) {
      ...
      const maxW = viewport.width * 0.7;
      const maxH = viewport.height * 0.55;
      let targetW = maxW;
    ...
    const handlePointerMove = (e: PointerEvent) => {
      ...
      if (isDraggingRef.current) {
        ...
        inertiaVelocityRef.current =
          (((e.clientY - lastPointerRef.current.y) * pxToWorld) / dt) * 16;
      ...
      const hits = raycastRef.current.intersectMeshes(
        slides.map((s) => s.mesh),
      );
      ...
      slides.forEach((slide) => {
        ...
        if (isHit) {
          ...
          const localX =
            (e.clientX / window.innerWidth - 0.5) * curCtx.viewport.width;
          const localY =
            (0.5 - e.clientY / window.innerHeight) * curCtx.viewport.height;
          slide.program.uniforms.uMouse.value = [
            Math.max(
              0,
              Math.min(
                1,
                (localX - slide.mesh.position.x) /
                  (slide.mesh.scale.x as number) +
                  0.5,
              ),
            ),
            Math.max(
              ...
              Math.min(
                1,
                (localY - slide.mesh.position.y) /
                  (slide.mesh.scale.y as number) +
                  0.5,
              ),
            ),
          ];
          gsap.to(slide.program.uniforms.uHover, {
            value: 1,
            duration: 0.4,
            ease: "power2.out",
            overwrite: true,
    ...
    const handlePointerUp = (e: PointerEvent) => {
      ...
      if (
        revealedRef.current &&
        hitSlide.projectIndex === revealedIndexRef.current
      ) {
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      ...
      if (renderTargetRef.current)
        renderTargetRef.current.setSize(canvasEl.width, canvasEl.height);
      if (postfxMeshRef.current)
        postfxMeshRef.current.scale.set(viewport.width, viewport.height, 1);
  ...
  return {
    ...
    getRevealedScreenRect: () => {
      ...
      const screenX =
        ((meshX - meshW / 2 + viewport.width / 2) / viewport.width) * cw;
      const screenY =
        ((viewport.height / 2 - meshY - meshH / 2) / viewport.height) * ch;
    ...
    selectSlide: (index: number) => {
      ...
      if (revealedRef.current) {
        (collapseRevealRef.current?.() ?? Promise.resolve()).then(
          doJumpAndReveal,
        );
      } else {
        doJumpAndReveal();
